...
- Made remote weapon, shield, and bow sheath/unsheath synchronization reliable by reading BOTW's authoritative `GameROMPlayer + 0xb94` Hold/Equip/Invisible state instead of an unstable controller-chain byte, and canonicalized the serialized equipment-state byte to `0` or `1` while accepting nonzero values from older clients.
