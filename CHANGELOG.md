...
- Fixed one-way remote-player visibility on Linux by making actor-spawn submission core-safe. The original PPC hook stored one global request flag while its prepared argument registers remained local to the callback's emulated core; Linux's multicore recompiler could therefore let a different core consume the request and invoke BOTW's actor factory with unrelated registers. The transfer ABI now includes the submitting stack as an ownership token, the PPC hook leaves mismatched requests pending, and only the exact originating invocation can consume and clear its request. Bundled patch contents are now included in the installed-mod signature so launcher updates automatically redeploy changed PPC patches instead of incorrectly retaining an older merge.
