...
MemoryAccess::LocalInstance* Game::GameInstance = nullptr;
...
{
    const float x = Link->Position[0] - Jugadores[playerID]->Pos[0];
    const float y = Link->Position[1] - Jugadores[playerID]->Pos[1];
    const float z = includeZAxis ? Link->Position[2] - Jugadores[playerID]->Pos[2] : 0.0f;
...
void Main::mainServerLoop()
{
    ...
    while (true)
    {
        ...
        if (!client->sendBytes(&serverData[0]))
        {
            Main::disconnectFromServer("Connection to the server was lost while sending data.");
            return;
        }
