...
namespace BOTW.DedicatedServer
{
    ...
    public class DedicatedServer
    {
        ...
        public DedicatedServer(string? selectedConfigPath = null)
        {
            baseDirectory = AppContext.BaseDirectory;
            configPath = selectedConfigPath ?? Path.Combine(baseDirectory, "ServerConfig.ini");
            LandmarkPositions = JsonConvert.DeserializeObject<Dictionary<string, Vec3f>>(
                File.ReadAllText(Path.Combine(baseDirectory, "Landmarks.json"))) ?? new();
            ServerProphuntLocations = JsonConvert.DeserializeObject<List<ProphuntLocation>>(
                File.ReadAllText(Path.Combine(baseDirectory, "PropHuntLocations.json"))) ?? new();
        }

        public bool setup(bool nonInteractive = false)
        {
            ServerConfig svConfig = new ServerConfig(configPath);

            ServerSettings settings = GetServerSettings(svConfig, nonInteractive);
            if (!server.serverStart(svConfig.Connection.IP, svConfig.Connection.Port, svConfig.Connection.Password, svConfig.ServerInformation.Description, settings))
                return false;
        ...
        public void Shutdown() => server.stopServer();

        private ServerSettings GetServerSettings(ServerConfig svConfig, bool nonInteractive = false)
        {
            if (svConfig.Gamemode.DefaultGamemode || nonInteractive)
            ...
            if (!Match)
            {
                if (Logger.LogInput("Do you wish to save your selected server settings? (1 for yes, 0 for no): ") == "1")
                {
                    ...
                    File.WriteAllText(Path.Combine(baseDirectory, "Gamemodes.json"), GamemodeJson);
        ...
        {
            string text = File.ReadAllText(PlatformPaths.DataFile("QuestFlagsNames.txt"));
            ...
            Gamemodes = JsonConvert.DeserializeObject<List<ServerSettings>>(File.ReadAllText(Path.Combine(baseDirectory, "Gamemodes.json"))) ?? new();
        ...
        {
            string AppdataFolder = PlatformPaths.DataDirectory;
            ...
            foreach (string resource in Resources)
            {
                ...
                if (s == null)
                    continue;
                const string marker = ".AppdataFiles.";
                int markerIndex = resource.IndexOf(marker, StringComparison.Ordinal);
                string resourceFileName = markerIndex >= 0 ? resource[(markerIndex + marker.Length)..] : Path.GetFileName(resource);
                string output = Path.Combine(AppdataFolder, resourceFileName);
                using (FileStream AppdataFile = new FileStream(output, FileMode.Create))
                {
                    s.CopyTo(AppdataFile);
                }
            }
        }
