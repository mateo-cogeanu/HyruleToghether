...
{
#ifndef _WIN32
    // Legacy scans use region 8 to mean Cemu's Wii U heap. Native Cemu maps
    // that heap as part of its reserved 4 GiB range instead of Windows'
    // eighth VirtualQuery region. Preserve offsets while removing the ordinal.
    const bool nativeCemuHeap = region == 8;
    // ScanOffset is an optimisation based on the allocation order used by the
    // old Windows build. Native Cemu does not preserve that ordering, and an
    // address can legitimately appear before the previous match. Always scan
    // the emulated reservation from its real base and never escape into the
    // rest of the host process address space.
...
#ifndef _WIN32
            if (nativeCemuHeap && nativeStartAddress > (uint64_t)Scanmbi.BaseAddress)
            {
                uint64_t requested = nativeStartAddress - (uint64_t)Scanmbi.BaseAddress;
                if (requested < Scanmbi.RegionSize)
                    startingAddress = requested;
            }
            if (nativeCemuHeap &&
                (uint64_t)Scanmbi.BaseAddress + endAddress > nativeEndAddress)
            {
                endAddress = nativeEndAddress - (uint64_t)Scanmbi.BaseAddress;
            }
#endif
