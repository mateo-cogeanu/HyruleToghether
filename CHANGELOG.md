...
- Added the animation synchronization lock's direct `<mutex>` dependency so generated Linux client sources compile `std::unique_lock` under GCC instead of relying on Apple Clang's transitive standard-library includes.
