...
namespace DataTypes
{
	...
	public:
		...
		EquippedItems()
		{
			...
			auto scan = [](const std::vector<int>& value) {
#ifdef _WIN32
				return Memory::PatternScan(value, Memory::getBaseAddress(), 8, Memory::ScanOffset);
#else
				return Memory::PatternScan(value, Memory::getBaseAddress());
#endif
			};

			uint64_t match = scan(signature);
			uint64_t addr = match ? match + 0x4 : 0;

			while (addr < 30000)
			{
				Sleep(2000);
				Logging::LoggerService::LogDebug("Failed to find porch.", __FUNCTION__);
				match = scan(signature);
				addr = match ? match + 0x4 : 0;
			}
