...
namespace DataTypes
{
	...
	public:
		...
		{
			int val = 0;
			auto* bytes = reinterpret_cast<byte*>(&val);
			memcpy(bytes, reinterpret_cast<void*>(addr + 2), 1);
			memcpy(bytes + 1, reinterpret_cast<void*>(addr + 1), 1);
			memcpy(bytes + 2, reinterpret_cast<void*>(addr), 1);
