...
- Fixed one-way remote-player visibility on Linux by making actor-spawn submission core-safe. The original PPC hook stored one global request flag while its prepared argument registers remained local to the callback's emulated core; Linux's multicore recompiler could therefore let a different core consume the request and invoke BOTW's actor factory with unrelated registers. The first ownership implementation compared the callback-visible stack pointer, but Linux's x86 HLE trampoline can expose a temporary `r1`, so valid requests remained permanently pending. Each submission now carries a unique token in saved register `r12`; only the exact originating invocation can consume and clear it, and the wrapper restores the game's original `r12` afterward. Bundled patch contents remain part of the installed-mod signature so launcher updates automatically redeploy changed PPC patches.
- Fixed a Linux `SIGSEGV` introduced while diagnosing remote actor creation. The original `patch_SpawnActors.asm` transfer block was 44 bytes including two implicit alignment bytes—not 42 bytes—and is now deliberately extended to 48 bytes by the cross-core owner token. The matching reversed native padding is retained, exact size/offset assertions document and enforce the PPC ABI, invalid ring-buffer addresses are rejected before dereferencing, and instance storage is read only when an actor is actually queued. Scoped packing still prevents this ABI alignment from leaking into unrelated native types.
