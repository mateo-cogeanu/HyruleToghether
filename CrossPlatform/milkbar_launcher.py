...
def install_bundled_mod(config: dict[str, Any], force: bool = False) -> Path:
    ...
    active_title_bg = output_pack / "content" / "Pack" / "TitleBG.pack"
    if not _file_contains_all(active_title_bg, REQUIRED_MULTIPLAYER_GAME_DATA):
        raise RuntimeError("The patched TitleBG.pack is missing multiplayer animation controls")
    for patch in patches:
