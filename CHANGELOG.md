...
- Fixed the Linux spawn hook reaching “submitted” without ever creating an actor. The native spawn-transfer structure was 44 bytes while `patch_SpawnActors.asm` defines a 42-byte block; whole-structure endian reversal therefore shifted its function address, ring pointer, and saved registers. The obsolete padding is removed, exact size/offset assertions now enforce the PPC ABI, and packed alignment no longer leaks into unrelated native types.
