...
public:
	...
	*/
	...
	template <typename T>
	uintptr_t GetAddressFromSignature(std::vector<T> signature, uintptr_t startaddress = 0, uintptr_t endaddress = 0) {
		SYSTEM_INFO si;
		GetSystemInfo(&si);
		if (startaddress == 0) {
			startaddress = reinterpret_cast<uintptr_t>(si.lpMinimumApplicationAddress);
		}
		else {
			startaddress = baseAddr + startaddress;
		}
		if (endaddress == 0) {
			endaddress = reinterpret_cast<uintptr_t>(si.lpMaximumApplicationAddress);
		}
		else {
		...
		for (uintptr_t i = startaddress; i < endaddress - signature.size(); i++) {
			...
			if (VirtualQuery((LPCVOID)i, &mbi, sizeof(mbi))) {
				...
				for (uintptr_t k = reinterpret_cast<uintptr_t>(mbi.BaseAddress); k < reinterpret_cast<uintptr_t>(mbi.BaseAddress) + mbi.RegionSize - signature.size(); k++) {
