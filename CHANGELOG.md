...
- Fixed remote melee weapon, shield, and bow synchronization at BOTW's actual equipment-child creation boundary. The native client now resolves each unique `Jugador` `NpcEquipment` placeholder to the received `Weapon_*` resource immediately before the actor factory performs its lookup, verifies/logs the resulting child actor, and keeps the existing one-shot controlled reload for later equipment changes without touching the working animation, armour, visibility, or spawn paths.
...
- Restored the established raw held/equipped byte semantics after the first factory-resolver build inverted that value and could make charged weapon animations lose their upper-body animation while leg movement continued.
