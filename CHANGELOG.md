...
- Fixed the macOS equipment-reload crash caused by a retry writing through an actor-local weapon pointer after BOTW deleted that actor. Equipment retries are now blocked for the duration of a controlled refresh, cached template lists explicitly exclude actor-local strings, and every cached write is checked against the live Cemu mapping before use.
- Restored melee weapon, shield, and bow visuals without changing the working animation path or actor lifecycle. Equipment resource names are now staged into every persistent loaded `NpcEquipment` template before the existing controlled reload, the dedicated third equipment slot is used for bows, and empty equipment is represented by an empty resource rather than an invalid `Weapon_*_000` actor.
