...
namespace MemoryAccess
{
	...
	public:
		...
		void scan()
		{
			...
			Logging::LoggerService::LogDebug("Scan stage complete: location", __FUNCTION__);
			Equipment = new EquippedItems();
			Logging::LoggerService::LogDebug("Scan stage complete: equipment", __FUNCTION__);
			...
			Logging::LoggerService::LogDebug("Scan stage complete: world day", __FUNCTION__);
			...
			Logging::LoggerService::LogDebug("Scan stage complete: core player state", __FUNCTION__);
			...
			Logging::LoggerService::LogDebug("Scan stage complete: multiplayer flags", __FUNCTION__);
		...
		std::map<int, FlagAddresses> scanPlayerFlags()
		{
			...
			// Match the stable flag name rather than the preceding BYML string-length
			// byte. UKMM may rewrite that serialization prefix while leaving the
			// actual GameData flag and the contiguous player-flag layout unchanged.
			sig = { 0x4A, 0x75, 0x67, 0x61, 0x64, 0x6F, 0x72, 0x31, 0x5F, 0x48, 0x6F, 0x6C, 0x64 }; // Jugador1_Hold

			addr = Memory::PatternScan(sig, Memory::getBaseAddress(), 8);
			//uint64_t addr_copy = addr;

			if (addr < 30000)
			{
				Logging::LoggerService::LogWarning("Optional animation/equipment/map flags are unavailable; continuing with core multiplayer synchronization.", __FUNCTION__);
				Logging::LoggerService::LogInformation("Scanned core game flags successfully.", __FUNCTION__);
				return result;
			}
	...
	private:
		...
		std::map<std::string, int> readWeaponDamages()
		{
			...
			std::ifstream file(std::string(appdata) + "\\BOTWM\\WeaponDamages.txt");
#else
			std::ifstream file(std::filesystem::path(MilkBarDataDirectory()) / "WeaponDamages.txt");
#endif
