...
- Replaced the lossy equipment boolean relay with the complete byte from Link's already-verified controller field without changing the packet footprint. Matching clients and servers now preserve values such as `0x57` end to end, older endpoints remain compatible through zero/nonzero semantics, and the remote actor still receives only the proven Hold/Equip transition when that normalized polarity changes. Added change-only sender, server-relay, receiver, and 17-byte controller-context diagnostics with guest/host addresses; neighboring bytes are observational only and do not drive speculative shield, bow, projectile, or damage behavior.
- Made remote weapon, shield, and bow sheath/unsheath synchronization reliable by reading Link's proven controller-chain state as a raw byte instead of an invalid C++ `bool` and mapping its visually verified polarity (`nonzero` held, zero sheathed). The remote `Jugador + 0xb94` Hold/Equip state remains isolated to the verified receiver-side actor path; diagnostics record the exact local controller address, raw flags, logical state, received wire byte, and remote actor readback.
...
- Kept the `Demo_ChangeEquipState` protocol semantics explicit: a nonzero `EquipmentState` requests `Hold`, while zero requests `Equip`; the full raw controller byte is retained separately from that normalized actor operation.
