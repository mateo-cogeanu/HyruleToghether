...
void LoggerService::StartLoggerService()
{
...
#else
    const std::filesystem::path dataDirectory(MilkBarDataDirectory());
    const std::filesystem::path logsDirectory = dataDirectory / "Logs";
    const std::filesystem::path latestLog = dataDirectory / "LatestLog.txt";
    ...
    if (std::filesystem::exists(latestLog)) {
        ...
        do {
            archived = logsDirectory / (LoggerService::GetTimeAsString(2) + "_" + std::to_string(counter++) + ".txt");
        } while (std::filesystem::exists(archived));
        std::filesystem::rename(latestLog, archived);
    }
    LogFile.open(latestLog, std::ios_base::app);
#endif
