...
namespace DataTypes
{
	...
	public:
		...
		LocationAccess()
		{
			...
			// Windows Cemu exposed the emulated heap as its eighth VirtualQuery
			// region. Native macOS/Linux reserve it differently, so a region ordinal
			// skips the actual Wii U memory. Scan committed emulated memory directly.
#ifdef _WIN32
			uint64_t addr = Memory::PatternScan(signature, Memory::getBaseAddress(), 8, Memory::ScanOffset);
#else
			uint64_t addr = Memory::PatternScan(signature, Memory::getBaseAddress());
#endif
...
#else
				addr = Memory::PatternScan(signature, Memory::getBaseAddress());
#endif
			}
