...
- Fixed the Linux crash at `setupActor+0x13a` revealed by the replacement-spawn log. The native client now captures the first proven-good actor-factory template and its storage bytes once, then reuses that stable host copy instead of dereferencing an unrelated actor's short-lived `r7` after an equipment deletion.
